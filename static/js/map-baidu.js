...
function removePokemonMarker (encounterId) { // eslint-disable-line no-unused-vars
  ...
  map.removeOverlay(mapData.pokemons[encounterId].marker);
...
function getBaiduSprite (index, sprite, displayHeight) {
  var scaledIconSize = new BMap.Size(sprite.iconWidth, sprite.iconHeight)
  var scaledIconOffset = new BMap.Size(
    - ((index % sprite.columns) * sprite.iconWidth),
    - (Math.floor(index / sprite.columns) * sprite.iconHeight))

  return {
    url: sprite.filename,
    size: scaledIconSize,
    anchor: scaledIconOffset
  }
}
...
function setupPokemonMarker (item, skipNotification, isBounceDisabled) {
  ...
  var bsprite = getBaiduSprite(pokemonIndex, sprite, iconSize)
  ...
  if (isBounceDisabled === true) {
   ...
   var icon = new BMap.Icon(bsprite.url, new BMap.Size(bsprite.size.width, bsprite.size.height));
   icon.setImageOffset(bsprite.anchor);
...
*/
  ...
  //marker.infoWindow = new BMap.InfoWindow(pokemonLabel(eng2chs(item['pokemon_name']), item['pokemon_rarity'], item['pokemon_types'], item['disappear_time'], item['pokemon_id'], item['latitude'], item['longitude'], item['encounter_id']))
...
var trueLoc = transformFromWGSToGCJ(item['longitude'], item['latitude']);
trueLoc = gcj02_To_Bd09(trueLoc.lat, trueLoc.lng);
item['longitude'] = trueLoc.lng;
item['latitude'] = trueLoc.lat;	

  var icon = new BMap.Icon('static/forts/' + gymTypes[item['team_id']] + '.png', new BMap.Size(36, 36));
  var marker = new BMap.Marker(new BMap.Point(item['longitude'], item['latitude']));
   marker.setIcon(icon)
   marker.setZIndex(2);
   map.addOverlay(marker);
...
item['latitude'] = trueLoc.lat;	
   ...
   var imagename = item['lure_expiration'] ? 'PstopLured' : 'Pstop'
   var icon = new BMap.Icon('static/forts/' + imagename + '.png', new BMap.Size(31, 31));
   var marker = new BMap.Marker(new BMap.Point(item['longitude'], item['latitude']));
   marker.setIcon(icon);
   marker.setZIndex(2);
   map.addOverlay(marker);
...
function setupScannedMarker (item) {
  ...
  */
  ...
  var marker = new BMap.Circle(point, 70);
...
item['latitude'] = trueLoc.lat;	
  ...
  var point = new BMap.Point(item['longitude'], item['latitude'])
  var marker = new BMap.Circle(point, 5);
...
function clearStaleMarkers () {
  $.each(mapData.pokemons, function (key, value) {
    if (mapData.pokemons[key]['disappear_time'] < new Date().getTime() ||
	  ...
	  map.removeOverlay(mapData.pokemons[key].marker)
  ...
  $.each(mapData.lurePokemons, function (key, value) {
    if (mapData.lurePokemons[key]['lure_expiration'] < new Date().getTime() ||
	  ...
	  map.removeOverlay(mapData.lurePokemons[key].marker);
  ...
  $.each(mapData.scanned, function (key, value) {
    ...
    if (mapData.scanned[key]['last_modified'] < (new Date().getTime() - 15 * 60 * 1000)) {
	  ...
	  map.removeOverlay(mapData.scanned[key].marker)
  ...
  $.each(markers, function (key, value) {
    ...
    if (!markers[key].hidden) {
      if (typeof marker.getBounds === 'function') {
        ...
        if (map.getBounds().containsPoint(marker.getPosition())) {
          show = true
        }
      }
    }
...
function processPokestops (i, item) {
  ...
  if (Store.get('showLuredPokestopsOnly') && !item['lure_expiration']) {
    if (mapData.pokestops[item['pokestop_id']] && mapData.pokestops[item['pokestop_id']].marker) {
	  ...
	  map.removeOverlay(mapData.pokestops[item['pokestop_id']].marker);
  ...
  if (!mapData.pokestops[item['pokestop_id']]) { // add marker to map and item to dict
    ...
    if (!!item['lure_expiration'] !== !!item2['lure_expiration']) {
	  ...
	  map.removeOverlay(item2.marker);
      item.marker = setupPokestopMarker(item)
      mapData.pokestops[item['pokestop_id']] = item
    }
  }
...
item['latitude'] = trueLoc.lat;	
  ...
  if (scanId in mapData.scanned) {
    ...
    //  fillColor: getColorByDate(item['last_modified'])
    //})
	mapData.scanned[scanId].marker.setFillColor(getColorByDate(item['last_modified']));
  } else { // add marker to map and item to dict
    if (item.marker) {
      //item.marker.setMap(null)
	  map.removeOverlay(item.marker);
      ...
      if (this.checked) {
        ...
        $.each(dataType, function (d, dType) {
          $.each(data[dType], function (key, value) {
			...
			map.removeOverlay(data[dType][key].marker);
          })
          data[dType] = {}
        })
      }
