...
function initMap () { // eslint-disable-line no-unused-vars
  map = new AMap.Map('map', {
    ...
    mapTypeControlOptions: {
      style: AMap.MapTypeControlStyle.DROPDOWN_MENU,
      position: AMap.ControlPosition.RIGHT_TOP,
      mapTypeIds: [
        AMap.MapTypeId.ROADMAP,
        AMap.MapTypeId.SATELLITE,
        'nolabels_style',
        'dark_style',
        'style_light2',
        'style_pgo',
  ...
  var styleNoLabels = new AMap.StyledMapType(noLabelsStyle, {
    name: 'No Labels'
  })
  map.mapTypes.set('nolabels_style', styleNoLabels)

  var styleDark = new AMap.StyledMapType(darkStyle, {
    name: 'Dark'
  })
  map.mapTypes.set('dark_style', styleDark)

  var styleLight2 = new AMap.StyledMapType(light2Style, {
    name: 'Light2'
  })
  map.mapTypes.set('style_light2', styleLight2)

  var stylePgo = new AMap.StyledMapType(pGoStyle, {
    name: 'PokemonGo'
  })
  map.mapTypes.set('style_pgo', stylePgo)

  var styleDarkNl = new AMap.StyledMapType(darkStyleNoLabels, {
    name: 'Dark (No Labels)'
  })
  map.mapTypes.set('dark_style_nl', styleDarkNl)

  var styleLight2Nl = new AMap.StyledMapType(light2StyleNoLabels, {
    name: 'Light2 (No Labels)'
  })
  map.mapTypes.set('style_light2_nl', styleLight2Nl)

  var stylePgoNl = new AMap.StyledMapType(pGoStyleNoLabels, {
  ...
  AMap.event.addListener(map, 'idle', updateMap)

  marker = createSearchMarker()

  addMyLocationButton()
  initSidebar()
  AMap.event.addListenerOnce(map, 'idle', function () {
    updateMap()
  })

  AMap.event.addListener(map, 'zoom_changed', function () {
...
function createSearchMarker () {
  var marker = new AMap.Marker({ // need to keep reference.
    position: {
      lat: centerLat,
      lng: centerLng
    },
    map: map,
    animation: AMap.Animation.DROP,
    draggable: !Store.get('lockMarker'),
    zIndex: AMap.Marker.MAX_ZINDEX + 1
  })
...
function initSidebar () {
  ...
  var searchBox = new AMap.places.SearchBox(document.getElementById('next-location'))
...
function pokemonLabel (name, rarity, types, disappearTime, id, latitude, longitude, encounterId) {
  ...
  var rarityDisplay = rarity ? '(' + rarity2chs(rarity) + ')' : ''
  var typesDisplay = ''
  ...
  var contentstring = `
    <div>
      ...
      <span> - </span>
      <small>
        <a href='http://www.pokemon.com/us/pokedex/${id}' target='_blank' title='View in Pokedex'>编号#${id}</a>
      </small>
      <span> ${rarityDisplay}</span>
      <span> - </span>
      <small>属性:${typesDisplay}</small>
    </div>
    <div>
      将在 ${pad(disappearDate.getHours())}:${pad(disappearDate.getMinutes())}:${pad(disappearDate.getSeconds())} 消失
      倒计时:<span class='label-countdown' disappears-at='${disappearTime}'>(00m00s)</span>
    </div>
    <div>
      坐标: ${latitude.toFixed(6)}, ${longitude.toFixed(7)}
    </div>
    <div>
      <a href='javascript:excludePokemon(${id})'>不要再显示</a>&nbsp;&nbsp
      <a href='javascript:notifyAboutPokemon(${id})'>再次出现通知我</a>&nbsp;&nbsp
      <a href='javascript:removePokemonMarker("${encounterId}")'>马上消失</a>&nbsp;&nbsp
      <a href='https://www.google.com/maps/dir/Current+Location/${latitude},${longitude}?hl=en' target='_blank' title='View in Maps'>地图上看这里</a>
    </div>`
...
function getGoogleSprite (index, sprite, displayHeight) {
  ...
  var scaledIconSize = new AMap.Size(scale * sprite.iconWidth - 1, scale * sprite.iconHeight - 1)
  var scaledIconOffset = new AMap.Point(
    (index % sprite.columns) * sprite.iconWidth * scale + 0.5,
    Math.floor(index / sprite.columns) * sprite.iconHeight * scale + 0.5)
  var scaledSpriteSize = new AMap.Size(scale * sprite.spriteWidth, scale * sprite.spriteHeight)
  var scaledIconCenterOffset = new AMap.Point(scale * sprite.iconWidth / 2, scale * sprite.iconHeight / 2)
...
function setupPokemonMarker (item, skipNotification, isBounceDisabled) {
  ...
  marker.infoWindow = new AMap.InfoWindow({
    content: pokemonLabel(eng2chs(item['pokemon_name']), item['pokemon_rarity'], item['pokemon_types'], item['disappear_time'], item['pokemon_id'], item['latitude'], item['longitude'], item['encounter_id']),
    disableAutoPan: true
  })
  ...
  if (notifiedPokemon.indexOf(item['pokemon_id']) > -1 || notifiedRarity.indexOf(item['pokemon_rarity']) > -1) {
    if (!skipNotification) {
      ...
      sendNotification('A wild ' + eng2chs(item['pokemon_name']) + ' appeared!', 'Click to load map', 'static/icons/' + item['pokemon_id'] + '.png', item['latitude'], item['longitude'])
    }
    if (marker.animationDisabled !== true) {
      marker.setAnimation(AMap.Animation.BOUNCE)
    }
  }
...
function setupScannedMarker (item) {
  var circleCenter = new AMap.LatLng(item['latitude'], item['longitude'])
...
function addListeners (marker) {
  ...
  AMap.event.addListener(marker.infoWindow, 'closeclick', function () {
    marker.persist = null
  })
...
var trueLoc = transformFromWGSToGCJ(item['longitude'], item['latitude']);
item['longitude'] = trueLoc.lng;
item['latitude'] = trueLoc.lat;	
...
function getPointDistance (pointA, pointB) {
  return AMap.geometry.spherical.computeDistanceBetween(pointA, pointB)
}
...
function myLocationButton (map, marker) {
  ...
  map.controls[AMap.ControlPosition.RIGHT_BOTTOM].push(locationContainer)
}
...
function centerMapOnLocation () {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function (position) {
      var latlng = new AMap.LatLng(position.coords.latitude, position.coords.longitude)
...
function changeLocation (lat, lng) {
  var loc = new AMap.LatLng(lat, lng)
...
$(function () {
  ...
  window.setInterval(updateMap, 1500)
  window.setInterval(function () {
    if (navigator.geolocation && Store.get('geoLocate')) {
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        if (getPointDistance(marker.getPosition(), (new AMap.LatLng(lat, lon))) > 40) {
          $.post(baseURL + '/next_loc?lat=' + lat + '&lon=' + lon).done(function () {
            var center = new AMap.LatLng(lat, lon)
            map.panTo(center)
